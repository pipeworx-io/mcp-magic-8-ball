...
`tools/list` at `https://gateway.pipeworx.io/magic-8-ball/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/magic_8_ball_ask \
  -H 'Content-Type: application/json' \
  -d '{"question":"Will this project be completed on time?"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/magic_8_ball_ask`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "magic-8-ball": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-magic-8-ball"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-magic-8-ball
```
